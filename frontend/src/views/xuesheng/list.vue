        ...
        <div>
            <el-card class="box-card">
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <!-- 序号 -->
                    <el-table-column prop="xuehao" label="学号" width="130">
                        <template #default="{row}"> {{ row.xuehao }} </template>
                    </el-table-column>
                    ...
                    <el-table-column width="80" label="审核">
                        <template #default="{row}">
                            {{ row.issh }}
                            <el-button size="small" :type="row.issh == '是' ? 'success' : 'danger'" @click="canXueshengCheckIssh(row)">
                                {{ row.issh == '是' ? '取消审核' : '审核' }}
                            </el-button>
                        </template>
                    </el-table-column>
                        ...
                        <template #default="{row}">
                            <el-button-group>
                                <el-tooltip effect="dark" content="详情" placement="top-start"
                                    ><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/xuesheng/detail?id='+row.id)">详情</el-button>
                                </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/xuesheng/updt?id='+row.id)">编辑</el-button>
                                </el-tooltip>
...
<script setup>
    ...
    import { canXueshengSelect, useXueshengSelect, canXueshengDelete, canXueshengCheckIssh } from "@/module";
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canXueshengDelete(ids).then((res) => {
                        if (res.code == 0) {
    ...
    const loadList = (page) => {
        ...
        http.post("/xuesheng/admin/list/", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
