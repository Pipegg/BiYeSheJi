<template>
    <div class="views-buzhizuoye-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 布置作业查询 </span>
                    </div>
                </template>
                <div class="form-search">
                    <el-form @submit.prevent.stop :inline="true" size="small">
                        ...
                        <el-form-item label="作业编号">
                            <el-input v-model="search.zuoyebianhao"></el-input>
                        </el-form-item>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="zuoyebianhao" label="作业编号" width="130">
                        <template #default="{row}"> {{ row.zuoyebianhao }} </template>
                    </el-table-column>
                    <el-table-column prop="jiezhiriqi" label="截至日期" width="120">
                        <template #default="{row}"> {{ row.jiezhiriqi }} </template>
                    </el-table-column>
                    ...
                    <el-table-column sortable="custom" prop="yitijiaorenshu" label="已提交人数" width="80">
                        <template #default="{row}"> {{ row.yitijiaorenshu }} </template>
                    </el-table-column>
                        ...
                        <template #default="{row}">
                            <el-button-group>
                                ...
                                <el-tooltip effect="dark" content="详情" placement="top-start"
                                    ><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/buzhizuoye/detail?id='+row.id)">详情</el-button>
                                </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/buzhizuoye/updt?id='+row.id)">编辑</el-button>
                                </el-tooltip>
...
<script setup>
    ...
    import { canBuzhizuoyeSelect, useBuzhizuoyeSelect, canBuzhizuoyeDelete } from "@/module";
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canBuzhizuoyeDelete(ids).then((res) => {
                        if (res.code == 0) {
    ...
    const loadList = (page) => {
        ...
        http.post("/buzhizuoye/admin/fabujiaoshi/", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
