<template>
    <div class="views-kechengshipin-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 课程视频查询 </span>
                    </div>
                </template>
                ...
                <div class="form-search">
                    <el-form @submit.prevent.stop :inline="true" size="small">
                        ...
                        <el-form-item label="视频名称">
                            <el-input v-model="search.shipinmingcheng"></el-input>
                        </el-form-item>
                        <el-form-item>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="shipinmingcheng" label="视频名称">
                        <template #default="{row}"> {{ row.shipinmingcheng }} </template>
                    </el-table-column>
                    <el-table-column prop="shipin" label="视频">
                        <template #default="{row}"> <e-video-list :src="row.shipin"></e-video-list> </template>
                    </el-table-column>
                    <el-table-column prop="xuexicishu" label="学习次数" width="80">
                        <template #default="{row}"> {{ row.xuexicishu }} </template>
                    </el-table-column>
                    ...
                    <el-table-column label="操作" fixed="right" width="180">
                        <template #default="{row}">
                            <el-button-group>
                                <el-button type="primary" :icon="Plus" size="small" @click="$router.push('/admin/xuexijilu/add?id='+row.id)">添加学习记录</el-button>

                                <el-button type="default" :icon="Search" size="small" @click="$router.push('/admin/xuexijilu?kechengshipinid='+row.id)">查询学习记录</el-button>

                                <el-tooltip effect="dark" content="详情" placement="top-start"
                                    ><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/kechengshipin/detail?id='+row.id)">详情</el-button>
                                </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/kechengshipin/updt?id='+row.id)">编辑</el-button>
                                </el-tooltip>
...
<script setup>
    ...
    import { canKechengshipinSelect, useKechengshipinSelect, canKechengshipinDelete } from "@/module";
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canKechengshipinDelete(ids).then((res) => {
                        if (res.code == 0) {
    ...
    const loadList = (page) => {
        ...
        http.post("/kechengshipin/admin/fabujiaoshi/", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
