<template>
    <div class="views-kechengxinxi-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 课程信息查询 </span>
                    </div>
                </template>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="kechengfengmian" label="课程封面" width="100">
                        <template #default="{row}"> <e-img :src="row.kechengfengmian" style="max-width: 120px" /> </template>
                    </el-table-column>
                    <el-table-column prop="kechengyaodian" label="课程要点" width="130">
                        <template #default="{row}"> {{ row.kechengyaodian }} </template>
                    </el-table-column>
                    ...
                    <el-table-column width="80" label="审核">
                        <template #default="{row}"> {{ row.issh }} </template>
                    </el-table-column>

                    <el-table-column label="操作" fixed="right" width="180">
                        <template #default="{row}">
                            <el-button-group>
                                <el-button type="primary" :icon="Plus" size="small" @click="$router.push('/admin/kechengshipin/add?id='+row.id)">课程视频</el-button>



                                <el-button type="primary" :icon="Plus" size="small" @click="$router.push('/admin/kechengziyuan/add?id='+row.id)">课程资源</el-button>




                                <el-button type="primary" :icon="Plus" size="small" @click="$router.push('/admin/buzhizuoye/add?id='+row.id)">布置作业</el-button>

                                <el-tooltip effect="dark" content="详情" placement="top-start"
                                    ><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/kechengxinxi/detail?id='+row.id)">详情</el-button>
                                </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/kechengxinxi/updt?id='+row.id)">编辑</el-button>
                                </el-tooltip>
...
<script setup>
    ...
    import { canKechengxinxiSelect, useKechengxinxiSelect, canKechengxinxiDelete, canKechengxinxiCheckIssh } from "@/module";
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canKechengxinxiDelete(ids).then((res) => {
                        if (res.code == 0) {
    ...
    const loadList = (page) => {
        ...
        http.post("/kechengxinxi/admin/fabujiaoshi/", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
