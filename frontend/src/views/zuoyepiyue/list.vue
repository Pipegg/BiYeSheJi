                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column label="操作" width="200" align="center">
                        <template #default="scope">
                            ...
                            <el-button type="success" size="small" @click="viewAIEvaluation(scope.row)">AI批改</el-button>
                        </template>
                    </el-table-column>
                </el-table>
...
<script setup>
    ...
    const loadList = (page) => {
        ...
        http.post("/zuoyepiyue/admin/list/", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
    ...
    // 查看AI批改结果
    const viewAIEvaluation = (row) => {
        router.push({
            path: '/admin/zuoyepiyue/detail',
            query: {
                id: row.id
            }
        })
