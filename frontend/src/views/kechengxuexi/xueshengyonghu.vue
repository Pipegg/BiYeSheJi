<template>
    <div class="views-kechengxuexi-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 课程学习查询 </span>
                    </div>
                </template>
                    ...
                    <el-form @submit.prevent.stop :inline="true" size="small">
                        ...
                        <el-form-item label="课程分类">
                            ...
                            >
                                ...
                                >
                                <e-select-option type="option" module="kechengfenlei" value="id"
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="kechengfenlei" label="课程分类" width="80">
                        <template #default="{row}">
                            <e-select-view module="kechengfenlei" :value="row.kechengfenlei" select="id"
                    ...
                    <el-table-column prop="xuexizhuangtai" label="学习状态" width="80">
                        <template #default="{row}"> {{ row.xuexizhuangtai }}</template>
                    </el-table-column>
                    <el-table-column prop="addtime" label="学习时间">
                        ...
                        <template #default="{row}">
                            <el-button-group>
                                <el-button type="primary" :icon="Plus" size="small"
                                           v-if="row.xuexizhuangtai == '待学习' || row.xuexizhuangtai == '学习中'"      @click="$router.push('/admin/xuexijindu/add?id='+row.id)">学习进度
                                </el-button>

                                <el-tooltip effect="dark" content="详情" placement="top-start"
                                >
                                    <el-button type="info" :icon="InfoFilled" size="small"
                                               @click="$router.push('/admin/kechengxuexi/detail?id='+row.id)">详情
                                    </el-button>
                                </el-tooltip>
...
<script setup>
    ...
    import {canKechengxuexiSelect, useKechengxuexiSelect, canKechengxuexiDelete} from "@/module";
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canKechengxuexiDelete(ids).then((res) => {
                        if (res.code == 0) {
    ...
    const loadList = (page) => {
        ...
        http.post("/kechengxuexi/admin/xueshengyonghu/", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
