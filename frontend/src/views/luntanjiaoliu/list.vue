<template>
    <div class="views-luntanjiaoliu-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 论坛交流查询 </span>
                    </div>
                </template>
                    ...
                    <el-form @submit.prevent.stop :inline="true" size="small">
                        ...
                        <el-form-item label="分类">
                            <el-select v-model="search.fenlei"
                                ...
                                ><e-select-option type="option" module="luntanfenlei" value="id" label="fenleimingcheng"></e-select-option
                            ></el-select>
                        </el-form-item>
                        <el-form-item>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="biaoti" label="标题" width="130">
                        <template #default="{row}"> {{ row.biaoti }} </template>
                    </el-table-column>
                    <el-table-column prop="fenlei" label="分类" width="80">
                        <template #default="{row}"> <e-select-view module="luntanfenlei" :value="row.fenlei" select="id" show="fenleimingcheng"></e-select-view> </template>
                    </el-table-column>
                    <el-table-column prop="tupian" label="图片" width="100">
                        <template #default="{row}"> <e-img :src="row.tupian" style="max-width: 120px" /> </template>
                    </el-table-column>
                    <el-table-column sortable="custom" prop="huifushu" label="回复数" width="80">
                        <template #default="{row}"> {{ row.huifushu }} </template>
                    </el-table-column>
                    <el-table-column prop="faburen" label="发布人" width="180">
                        <template #default="{row}"> {{ row.faburen }} </template>
                    </el-table-column>
                    <el-table-column sortable="custom" prop="addtime" label="发布时间">
                    ...
                    <el-table-column width="80" label="审核">
                        <template #default="{row}">
                            {{ row.issh }}
                            <el-button size="small" :type="row.issh == '是' ? 'success' : 'danger'" @click="canLuntanjiaoliuCheckIssh(row)">
                                {{ row.issh == '是' ? '取消审核' : '审核' }}
                            </el-button>
                        </template>
                    </el-table-column>
                        ...
                        <template #default="{row}">
                            <el-button-group>
                                ...
                                <el-tooltip effect="dark" content="详情" placement="top-start"
                                    ><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/luntanjiaoliu/detail?id='+row.id)">详情</el-button>
                                </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/luntanjiaoliu/updt?id='+row.id)">编辑</el-button>
                                </el-tooltip>
...
<script setup>
    ...
    import { canLuntanjiaoliuSelect, useLuntanjiaoliuSelect, canLuntanjiaoliuDelete, canLuntanjiaoliuCheckIssh } from "@/module";
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canLuntanjiaoliuDelete(ids).then((res) => {
                        if (res.code == 0) {
    ...
    const loadList = (page) => {
        ...
        http.post("/luntanjiaoliu/admin/list/", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
