...
export const canXiaoxiSelect = (filter, result) => {
    http.post("/xiaoxi/index/").then((res) => {
        if (res.code == 0) {
...
export const canXiaoxiFindById = (id) => {
    return new Promise((resolve, reject) => {
        // 读取后台数据
        http.get("/xiaoxi/findById/", { id }).then((res) => {
            if (res.code == 0) {
                resolve(res.data);
            } else {
