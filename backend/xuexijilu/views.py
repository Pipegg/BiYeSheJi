...
from kechengshipin.models import Kechengshipin
# Create your views here.

from core import Blueprint,R

# 创建路由装饰器
router=Blueprint("xuexijilu")
...
def getWhere(request,qs):
    ...
    if utils.param("kechengshipinid"):
        qs = qs.filter(kechengshipinid=utils.param("kechengshipinid"))
    # 判断是否填写课程编号
...
def selectAll(request):
    # 获取搜素条件
    qs = getWhere(request,Xuexijilu.objects)
    # 获取所有行
...
def findById(request):
    ...
    id = utils.param("id")
    # 根据id 获取一行数据
    mmm = Xuexijilu.objects.get(pk = id)
    # 没有找到这行数据则返回没有找到相关数据
    if mmm == None:
...
def delete(request):
    ...
    for id in ids:
        # 获取一行数据
        map = Xuexijilu.objects.get(pk = id)
...
def insert(request):
    ...
    # 获取提交的数据
    data = {
        'kechengshipinid': utils.param('kechengshipinid',0),
...
def update(request):
    ...
    # 获取原有数据
    old = Xuexijilu.objects.get(pk = charuid)
    # 更新的数据
    data = {
        'id': charuid,
        'kechengshipinid': utils.param('kechengshipinid',old.kechengshipinid),
