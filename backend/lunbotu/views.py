...
def getWhere(request,qs):
    ...
    # 判断是否填写标题
    if utils.param("title"):
        qs = qs.filter(title__contains=utils.param("title"))
...
def update(request):
    ...
    data = {
        'id': charuid,
        'title': utils.param('title',old.title),
        'image': utils.param('image',old.image),
        'url': utils.param('url',old.url),
