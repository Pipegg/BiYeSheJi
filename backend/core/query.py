...
class Builder:
    ...
    def parseWhere(self):
        ...
        if 'where' in self.query.options:
            ...
            buffer = " WHERE "
            i=0
            # Protection against passing a raw Python list/dict 
            if isinstance(wheres, (list, dict)):
                print("WARNING: Raw Python object in WHERE clause, this should be processed earlier.")
                # This is a fallback in case the view-level processing fails
                if isinstance(wheres, list):
                    for item in wheres:
                        if isinstance(item, dict) and 'name' in item and 'value' in item:
                            ...
                            if i > 0:
                                buffer += " AND "
                            buffer += f" ( {name} {exp} '{value}' ) "
                            i += 1
                    if i == 0:  # If no valid conditions were processed
                        buffer = ""  # Reset to no WHERE clause
                    return buffer
    ...
    def formatString(self,value):
        # First convert value to string safely
        if value is None:
            return 'NULL'
        ...
        # For strings, properly escape SQL-specific characters
        val = str(value)
        # Replace backslash first to avoid double escaping
        val = val.replace('\\', '\\\\')
        # Then escape quotes
        val = val.replace("'", "\\'")
        # Also escape other potentially dangerous characters
        val = val.replace('"', '\\"')
